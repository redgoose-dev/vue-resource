...
<article class="page">
	...
	<p>
		스타일시트 변수들은 다음과 같은 섹션으로 분류되고 거기에 해당되는 값들을 지정한다.<br/>
		변수 이름은 섹션 이름을 프리픽스로 두고 정한다.
	</p>
	<pre><code>// color
$color-base: #222;
...
$size-min: 320px;</code></pre>
	...
	<p>
		<code>variables.scss</code> 파일은 한번 불러오면 <code>scss</code>가 연결된 내용이라면 언제나 사용할 수 있지만 IDE 프로그램에서는 그걸 제대로 인식하지 못하기 때문에 scss 파일마다 따로 import 해주는것을 권장한다. (그것이 정석인듯..)
	</p>
