<template>
<article class="page">
	<head-title type="h2" title="forms"/>
	...
	<p>
		<code>forms.scss</code> 파일에 들어있는 스타일시트들은 폼 관련 컴포넌트들을 그루핑하거나 보조하는 스타일들을 작성해서 관리한다.<br/>
		예를들어 <code>폼 항목의 필드, 필드셋, 폼에대한 도움말 및 오류 메시지..</code> 같은 부분들에서 사용하는 스타일시트를 만들어 사용한다.
	</p>
	<p>프로젝트에 따라 많은 조정이 필요한 부분이므로 스타일시트로만 만들어 사용하는것이 좋아 보인다.</p>
	<p>현재 파일에서는 다른 클래스와의 이름 구분을 위하여 <code>.forms-</code>이라는 프리픽스를 사용한다.</p>

	<section>
		<head-title type="h3" title="demo"/>
		<p>
			다음 예제는 `forms`에서 사용되는 클래스와 각 폼 컴포넌트들을 조합할때의 모습이다.<br/>
			이 예제는 데모를 위하여 약간 수정한것 뿐이고, 프로젝트에서 사용할 스타일을 상황에 맞게 고쳐야 할것이다.
		</p>
		<p>
	...
	<section>
		<head-title type="h3" title="forms-fieldset"/>
		<p>
			필드의 그룹이 되는 엘리먼트인 <code>fieldset</code>에서 사용한다. 글 작성폼에서 하나의 섹션이라고 볼 수 있다.<br/>
			`fieldset` 엘리먼트의 기본 스타일을 삭제하고 그 하위의 legend 엘리먼트를 안보이게 하는 역할을 한다.
		</p>
		<pre>
...
&lt;/fieldset></code>
	...
	<section>
		<head-title type="h3" title="forms-field"/>
		<p>
			글을 작성하는 폼 항목 하나를 표현할때 사용한다. 폼과 레이블, 도움말 메시지의 조합으로 이루어진다.
		</p>
		<pre>
...
&lt;/div></code>
	...
	<section>
		<head-title type="h3" title="forms-label"/>
		<p>
			폼을 설명하거나 이름이 되는 레이블에 대한 스타일이다. 통일된 레이블 스타일을 위하여 사용한다.
		</p>
		<pre><code>&lt;label for="example">Label name&lt;/label></code></pre>

		<head-title type="h3" title="forms-help"/>
		<p>
			현재 필드를 설명하는 텍스트 스타일이다.<br/>
			필드가 오류났거나 다른 상태에 대한 메시지를 표현하기도 한다.
		</p>
		<div class="example">
			<p class="forms-help">help message - basic</p>
